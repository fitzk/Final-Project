        ...
        .done(function(jsonObj) {
            if (jsonObj === "0 results") {
                ...
                data.forEach(function(object) {
                    var time1 = object['value'].split(':');
                    totalSecs = totalSecs + toSeconds(time1);
                });
                data.forEach(function(object) {
                    var time1 = object['value'].split(':');
                    object['value'] = toSeconds(time1) / totalSecs * 100;
                });
...
function showLog(data) {
	...
	data.forEach(function(objLog){
	  ...
	  inD++;
	  addCell(row,inD);
	  addCell(row,objLog['start']);
	  addCell(row,objLog['stop']);
	  addCell(row,"");
	});
